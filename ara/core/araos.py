...
ARAOS - BETA vers. 038
    Featuries:
 - system monitoring (cpu, memory and disc, network)
 - Automatic execution of remediation scripts
 - PyQt6 graphical interface with tabs: Dashboard, Code Creator, Architecture, Chat.
"""
...
# Importações para o novo sistema de chat
...
# --- NOVO: Executor seguro para o chat ---
class SafeExecutor:
...
class ARAChatSystem:
    ...
    def _train_basic(self):
        ...
        # Treinamento específico para comandos do sistema
        list_trainer = ListTrainer(self.chatbot)
        list_trainer.train([
            "Como está o sistema?", "Verificando status do sistema...",
            "Quais processos estão rodando?", "Listando processos ativos...",
            "Execute este código: print('Hello')", "Executando código Python...",
            "Limpe os temporários", "Iniciando limpeza de arquivos temporários...",
            "Reinicie o aplicativo", "Reiniciando sistema ARA..."
        ])
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.chat_executor = SafeExecutor()
        self.chat_system = ARAChatSystem(self.chat_executor)
    ...
    def _init_ui(self):
        ...
        h = QWidget(); self._build_chat(h);         self.tabs.addTab(h, "Chat")  # Nova aba
    ...
    # --- NOVO: Construir a interface do chat ---
    def _build_chat(self, w: QWidget):
        L = QVBoxLayout(w)
        ...
        self.chat_view.setFont(QFont("Consolas", 11))
        self.chat_view.setStyleSheet("background: #1e1e1e; color: #eee;")
        
        # Entrada de texto
        self.chat_input = QLineEdit()
        self.chat_input.setPlaceholderText("Digite uma mensagem ou !comando...")
        self.chat_input.setStyleSheet("background: #1e1e1e; color: #eee;")
        
        # Botão de envio
        send_btn = QPushButton("Enviar")
        send_btn.setFixedHeight(30)
        send_btn.setStyleSheet("background: #444; color: #ddd; border:1px solid #555;")
        send_btn.clicked.connect(self._on_chat_send)
        
        # Layout
        L.addWidget(self.chat_view, 1)  # O chat_view ocupa a maior parte
        input_layout = QHBoxLayout()
        input_layout.addWidget(self.chat_input, 1)  # O campo de entrada expande
        input_layout.addWidget(send_btn)            # Botão fixo
        L.addLayout(input_layout)
        
        # Mensagem inicial
        self.chat_view.append("ARA: Sistema de chat local inicializado!")
        self.chat_view.append("ARA: Digite !ajuda para ver os comandos disponíveis.")
        
        # Conectar Enter ao botão de envio
    ...
    def _on_chat_send(self):
        ...
        self.chat_view.append(f"> {query}")
        self.chat_input.clear()  # Limpa o campo
        
        try:
            # Obter resposta do sistema de chat
            response = self.chat_system.get_response(query)
            self.chat_view.append(f"ARA: {response}")
            ...
            self.chat_view.append(f"ERRO: {str(e)}")
...
def main():
    ...
    while True:
        ...
        win.show()
        exit_code = app.exec()
        if exit_code != 100:  # Código de reinício
            break
...
if __name__ == "__main__":
    ...
    try:
        ...
        subprocess.run([sys.executable, "-m", "pip", "install", "chatterbot", "chatterbot-corpus"])
        print("Dependências instaladas. Por favor execute o programa novamente.")
        sys.exit(0)
